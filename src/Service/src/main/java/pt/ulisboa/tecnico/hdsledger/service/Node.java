    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            LOGGER.log(Level.INFO,
                MessageFormat.format("Node {0}:\nNode Socket: {1}:{2}\nClient Socket: {3}:{4}\nIs Leader? {5}",
                                  nodeConfig.getId(),
                                  nodeConfig.getHostname(), nodeConfig.getPort(),
                                  nodeConfig.getHostname(), nodeConfig.getClientPort(),
                                  nodeConfig.isLeader()));
            ...
            NodeService nodeService = new NodeService(linkToNodes, nodeConfig, leaderConfig,
                    nodeConfigs);
            ClientService clientService = new ClientService(linkToClients, nodeConfig, leaderConfig, nodeConfigs,
                    nodeService);
