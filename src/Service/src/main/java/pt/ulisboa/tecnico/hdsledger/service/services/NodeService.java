  ...
  public void startConsensus(String value) {
    ...
    if (existingConsensus != null) {
      LOGGER.log(Level.INFO,
          MessageFormat.format("{0} - Node already started consensus for instance {1}",
    ...
    if (this.config.isLeader(localConsensusInstance, instance.getCurrentRound())) {
      ...
      LOGGER.log(Level.INFO, MessageFormat
          .format("{0} - Node is not leader, waiting for PRE-PREPARE message", config.getId()));
    }
  }
  ...
  public synchronized void uponPrepare(ConsensusMessage message) {
    ...
    if (instance.getPreparedRound() >= round) {
      ...
      ConsensusMessage m = new ConsensusMessageBuilder(config.getId(), Message.Type.COMMIT)
          ...
          .setMessage(instance.getCommitMessage().toJson()).build();
